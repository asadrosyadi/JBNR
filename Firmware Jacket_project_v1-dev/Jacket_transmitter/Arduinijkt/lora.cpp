...
static const char *TAG = "RFM9x";

static SPISettings loraSpiSettings(LORA_CLOCK_SPEED_MHZ * 1000000, MSBFIRST, SPI_MODE0);
...
esp_err_t spi_init(void)
{
    ...
    pinMode(LORA_PIN_RX_EN, OUTPUT);
    pinMode(LORA_PIN_TX_EN, OUTPUT);
    digitalWrite(LORA_PIN_RX_EN, LOW);
    digitalWrite(LORA_PIN_TX_EN, LOW);
...
void radio_init(void)
{
     ...
     * (see original ESP-IDF lora.c for the full parameter comment block)
     */
    setBandwidth(62.5e3);
    setCodingRate(4);
...
{
    uint8_t buffer[2] = { (uint8_t)reg, 0xff };
...
{
    uint8_t buffer[2] = { (uint8_t)(reg | SPI_WRITE_BIT_MASK), value };
...
void setFrequency(uint32_t frequency)
{
    ...
    double frequencyHz = frequency * 1000000.0;
    uint64_t frf = ((uint64_t)(frequencyHz) << 19) / (uint64_t)RFM9X_FXOSC;
...
uint32_t getFrecuency()
{
    ...
    frequency = (uint32_t)(((msb << 16) | (mid << 8) | lsb) * RFM9X_FXOSC / (1 << 19));
...
uint8_t getTxPower(void)
{
    ...
    if ((paConfig & 0x80) && paDac == 0x87) {
        // PA_BOOST + +20 dBm mode
        return 20;
    } else if (paConfig & 0x80) {
        // PA_BOOST, normal mode +2..17
        return 2 + (paConfig & 0x0F);
    } else {
        // RFO pin, limited to -4..15
...
void lora_send(uint8_t *data, uint8_t length)
{
    ...
    digitalWrite(LORA_PIN_TX_EN, HIGH);
    digitalWrite(LORA_PIN_RX_EN, LOW);
    ...
    TickType_t timeout = pdMS_TO_TICKS(3000);   // 3 seconds max

    while ((register_read(RFM9X_12_REG_IRQ_FLAGS) & IRQ_TX_DONE_MASK) == 0)
    {
        if ((xTaskGetTickCount() - start) >= timeout) {
            printf("LoRa TX timeout (3s)\n");
            break;
        }
...
void set_rx_enable() {
    digitalWrite(LORA_PIN_TX_EN, LOW);
    digitalWrite(LORA_PIN_RX_EN, HIGH);
}
