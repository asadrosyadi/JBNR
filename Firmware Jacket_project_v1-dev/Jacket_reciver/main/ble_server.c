...
static uint8_t vitals_char_str[] = {0x11, 0x22, 0x33};  // Combined HR & SpO2
static uint8_t location_char_str[] = {0x44, 0x55, 0x66}; // Combined Lat & Lon
...
// Service A: MAX30102 Event Handler (Combined HR & SpO2)
void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    switch (event) {
		...
		case ESP_GATTS_READ_EVT: {
		    ...
		    // Format the floats directly (removed float_to_string)
...
// Service B: GPS Event Handler (Combined Lat & Lon)
void gatts_profile_b_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    switch (event) {
		...
		case ESP_GATTS_READ_EVT: {
		    ...
		    // Reject long-read offsets (BLE max read is MTU based)
		    if (param->read.offset != 0) {
		        esp_ble_gatts_send_response(
		            gatts_if,
		            param->read.conn_id,
...
// Service C: Temperature Event Handler (unchanged)
...
// Service D: Text Event Handler (unchanged)
